...
int main() {
  ...
  boost::shared_ptr<vcl::matrix<double, vcl::row_major> >  n
    = matrix_init_ndarray<double, vcl::row_major>(array);
  ...
  */
  ...
  //boost::shared_ptr<viennacl::matrix<double> > n(&m);
  ...
  expr[0].lhs.matrix_row_double = &m; //n.get();
  ...
  expr[1].lhs.type_family = viennacl::scheduler::SCALAR_TYPE_FAMILY;
  expr[1].lhs.subtype = viennacl::scheduler::HOST_SCALAR_TYPE;
  expr[1].lhs.numeric_type = viennacl::scheduler::DOUBLE_TYPE;
  expr[1].lhs.host_double = 2.718;
  expr[1].op.type_family = viennacl::scheduler::OPERATION_BINARY_TYPE_FAMILY;
  expr[1].op.type = viennacl::scheduler::OPERATION_BINARY_MULT_TYPE;
  expr[1].rhs.type_family = viennacl::scheduler::MATRIX_TYPE_FAMILY;
  expr[1].rhs.subtype = viennacl::scheduler::DENSE_ROW_MATRIX_TYPE;
  expr[1].rhs.numeric_type = viennacl::scheduler::DOUBLE_TYPE;
  expr[1].rhs.matrix_row_double = &m; //n.get();
  ...
  //std::cout << bp::extract<const char*>(bp::str(vcl_matrix_to_ndarray<double, vcl::column_major>(*(m.get())))) << std::endl;
